...
with st.expander("Add or edit a flock transaction", expanded=False):
    ...
    edit_label = st.selectbox("Record", ["Create new", *transaction_options], key="flock_tx_record")
    current = {}
    ...
    flock_label = left.selectbox("Flock *", list(flock_labels), key="flock_tx_flock")
    transaction_type = left.selectbox("Transaction Type *", FLOCK_TRANSACTION_TYPES, key="flock_tx_type")
